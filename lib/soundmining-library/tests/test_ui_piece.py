...
from soundmining_library.ui.ui_piece_model import UiPiece, UiPieceBuilder, UiTrack
...
class TestUiPiece(unittest.TestCase):
    def test_build_piece(self) -> None:
        ...
        expected_piece = UiPiece([
            ...
            UiTrack("Track 2", [SequenceNote(0, "Track 2", duration=5, note=3, peak=0.33)]),
        ])
    ...
    def test_add_notes(self) -> None:
        ...
        piece = UiPieceBuilder().add_notes(sequence_notes).build()

        expected_piece = UiPiece([
            UiTrack("Track 1", [SequenceNote(0, "Track 1", duration=3, note=1, peak=0.5), SequenceNote(1, "Track 1", duration=3, note=2, peak=0.5)]),
            UiTrack(
                "Track 2",
                [
                    SequenceNote(0, "Track 2", duration=5, note=3, peak=0.33),
