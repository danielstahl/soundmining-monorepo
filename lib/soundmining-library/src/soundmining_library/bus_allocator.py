class BusAllocator:
    ...
    def _allocate_new_channels(self, nr_of_channels: int) -> tuple[int, ...]:
    ...
    def _add_new_channels_allocation(self, nr_of_channels, start: float, end: float) -> tuple[int, ...]:
        channels = self._allocate_new_channels(nr_of_channels)
        self.allocations[channels] = [(start, end)]
        return channels

    def _add_channel_allocation(self, channels: tuple[int], start: float, end: float) -> tuple[int, ...]:
    ...
    def allocate(self, nr_of_channels: int, start: float, end: float) -> tuple[int, ...]:
        for channels, allocs in self.allocations.items():
            if len(channels) == nr_of_channels and all(map(lambda x: not does_overlap(start, end, x[0], x[1]), allocs)):
                return self._add_channel_allocation(channels, start, end)
        return self._add_new_channels_allocation(nr_of_channels, start, end)


def does_overlap(start: float, end: float, alloc_start: float, alloc_end: float) -> bool:
    return is_between(start, alloc_start, alloc_end) or is_between(end, alloc_start, alloc_end) or (start <= alloc_start and end >= alloc_end)
